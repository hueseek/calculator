...
# PyQt5 -> PySide6, 애플리케이션 핸들러와 빈 GUI 위젯
# QMessageBox : 메시지박스 위젯
# QPlainTextEdit 추가
# QHBoxLayout 추가
from PySide6.QtWidgets import (QApplication, QWidget, QPushButton, QVBoxLayout, QMessageBox, QPlainTextEdit, QHBoxLayout) 
from PySide6.QtGui import QIcon     # icon을 추가하기 위한 라이브러리

class Calculator(QWidget):  # QWidget을 상속받아서 클래스 정의
    ...
    def initUI(self):
        ...
        # self.setWindowIcon(QIcon('icon.png'))   # 윈도우 아이콘 추가(Mac에서는 기본적으로 아이콘 비활성화)
        self.resize(256, 256)   # 윈도우 사이즈
        self.show()             # 윈도우를 화면에 표시되도록 호출
...
if __name__ == '__main__':      # PySide는 애클리케이션 당 1개의 QApplication이 필요함
    app = QApplication(sys.argv)    # QApplication 인스턴스 생성
    view = Calculator()             # Calculator 윈도우 인스턴스 생성
    sys.exit(app.exec())            # 애플리케이션이 이벤트 처리를 하도록 루프 구동
